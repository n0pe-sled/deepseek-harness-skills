...
3. Collect, then render. Put outputs somewhere outside the audited repository unless
   ...
   ```sh
   uv run scripts/collect.py <project-dir> --json <out-dir>/findings.json
   uv run scripts/render.py <out-dir>/findings.json --out <out-dir>/report.md
   ```
...
The `report-writing:writing-style` skill referenced in the body is not installed in DeepSeek Harness; follow the register described here. Run the scripts from this skill's directory (`scripts/collect.py`, `scripts/render.py`), treating the relative paths below as resolved from this directory or calling uv with an absolute path.
