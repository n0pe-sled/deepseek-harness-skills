...
4. **Zero findings needs investigation, not celebration.** It can mean poor extraction, missing models, the wrong packs, or suite filtering. Run `scripts/check_db_quality.py` after the build, confirm `scripts/verify_query_suite.py` exited zero for the suite in use — the generation scripts run it, so invoke it by hand only for a reused or hand-edited suite — and say in the report that both passed.
...
if ! DB_LIST=$("scripts/find_databases.sh" "${OUTPUT_DIR:-.}" .); then
...
- [ ] Database built (discovered via `codeql-database.yml` marker) and `scripts/check_db_quality.py` exited zero
- [ ] Data extensions evaluated — either created in `$OUTPUT_DIR/extensions/` or explicitly skipped with justification
- [ ] Analysis run with explicit suite reference (not default pack suite), and `scripts/verify_query_suite.py` exited zero for it
...
`/static-analysis:codeql-build` is a Claude Code workflow and is not available here. Run the database build and analysis steps from the body with Bash. The AskUserQuestion database-selection prompts become plain chat questions. Scripts and references resolve relative to this skill's directory.
