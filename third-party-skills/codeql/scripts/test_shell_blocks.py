...
def test_helper_uses_are_sourced_in_the_same_block() -> None:
    ...
    assert not offenders, (
        ...
        '"scripts/build_log.sh" || exit 1` to the block:\n  ' + "\n  ".join(offenders)
    )
...
HELPERS_MUST_PASS = (
    '. "scripts/build_log.sh" || exit 1\nlog_step "building"',
    "source scripts/build_log.sh\nrun_logged make",
...
def test_discovery_exit_status_is_observed() -> None:
    ...
    assert not offenders, (
        ...
        '`if ! DB_LIST=$("scripts/find_databases.sh" …); then … fi` and loop over '
        '`<<<"$DB_LIST"`:\n  ' + "\n  ".join(offenders)
    )


DISCOVERY_MUST_FLAG = (
    'done < <("scripts/find_databases.sh" "${OUTPUT_DIR:-.}" .)',
    "done < <(find_databases.sh)",
    # Spacing between the redirect and the substitution is the caller's habit, not a signal.
    'done <  <("scripts/find_databases.sh" .)',
)
