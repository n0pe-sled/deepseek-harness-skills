...
This skill was written for Claude Code's slash commands and named agents. The orchestration maps onto DeepSeek Harness tooling like this:

- Dispatch each function to a `subagent` (or `subagent_fork`) with the analysis format as its prompt. Have the subagent write its record to `audit-context/functions/<name>.md` and return only a short summary.
- After the subagents finish, write the `audit-context/DOSSIER.md` yourself in the session that owns the audit.
- Resource files under `resources/` resolve relative to this skill's directory.
