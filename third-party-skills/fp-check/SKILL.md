...
Deep verification in the body mentions "the plugin's analysis agents". DeepSeek Harness has no plugin agents: run the phases inline with Read, Grep, Glob, and Bash, and track the checklist with the `todo` tool. The gate reviews, evidence templates, and the TRUE POSITIVE / FALSE POSITIVE verdict format are unchanged. Reference files under `references/` resolve relative to this skill's directory.
