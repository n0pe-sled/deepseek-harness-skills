...
The `differential-review:adversarial-modeler` agent shipped with this skill in Claude Code, but not in DeepSeek Harness. For Phase 5 on a HIGH RISK change, delegate to a DSH `subagent` with the 5-step adversarial methodology (attacker model, attack vectors, exploitability rating, exploit scenario, baseline cross-reference). The `issue-writer` skill referenced in the body and in reporting.md is not installed here; write the report file directly, or open a GitHub issue if that is the deliverable. Support docs (methodology.md, adversarial.md, reporting.md, patterns.md) resolve relative to this skill's directory.
