...
```bash
# run-all
uv run --no-project scripts/merge_sarif.py "$OUTPUT_DIR/raw" "$OUTPUT_DIR/results/results.sarif" \
...
Claude-specific pieces: the `/static-analysis:semgrep-scan` workflow is not available. Work the five steps in the body directly. The approval gate that used AskUserQuestion becomes a plain chat prompt: present the ruleset plan and wait for explicit yes or no. Task tracking maps to the `todo` tool. Scripts and references resolve relative to this skill's directory (`scripts/run-scans.sh`, `references/rulesets.md`).
