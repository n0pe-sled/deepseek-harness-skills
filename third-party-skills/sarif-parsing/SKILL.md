...
`LEVEL_FN` definition in [resources/jq-queries.md](resources/jq-queries.md);
in Python it is `resolve_level(result, run)` in
[resources/sarif_helpers.py](resources/sarif_helpers.py).
...
Two SARIF fixtures live in [resources/fixtures](resources/fixtures),
